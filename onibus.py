...
class OnibusConvencional(Onibus):
    def __init__(self, modelo, fabricante, numrodas, descricao):
        ...
        self.__assentos = [[0]*12,[0]*12,[0]*12,[0]*12]

    def get_assentos(self):
        return self.__assentos

    def set_assentos(self, linha, coluna, passageiro):
        if 0 <= linha < len(self.__assentos) and 0 <= coluna < len(self.__assentos[0]):
            if self.__assentos[linha][coluna] == 0:
                self.__assentos[linha][coluna] = passageiro
                return True
        return False
